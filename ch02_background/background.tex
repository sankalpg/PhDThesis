...
In this section we review the existing approaches for melodic pattern processing in \gls{iam}. By melodic pattern processing we jointly refer to a number of research tasks that involve computational analysis of melodic patterns such as pattern similarity, pattern detection and pattern discovery. Depending on the musical relevance of the melodic patterns, they are at times referred to as melodic motifs in the literature. Analysis of melodic patterns or motifs is a well established research task in \gls{mir} for several music traditions~(Section XX). However, for \gls{iam} this computational task has gained attention only recently, mainly during the course of this dissertation. We summarize the existing approaches proposed for pattern processing in \gls{iam} in~\tabref{tab:pattern_processing_iam}. 
