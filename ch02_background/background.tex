...
In the scope of this thesis we group the different types of melodic patterns discussed above into three main categories, \gls{gamaka} type pattern (which also encapsulate different \glspl{alankar}), 
...
\TODO{If possible use some figures to explain the concept!}

\TODO{If time permits highlight the positives and little bit more detail what the approaches have done, and most of all what was their motivation, what were they trying to capture.}
...
\TODO{Add pictures to explain concepts, importance of transients, quantized version of a phrase, or some phrase examples}
...
\TODO{Mention in one paragraph the approaches that address pattern processing where the patterns are like gamakas and Alankars. Pratyush's work etc...In the proposal there is a paragraph on this topic}

DETECTION OF MICRO-TONAL ORNAMENTATIONS IN DHRUPAD USING A DYNAMIC PROGRAMMING APPROACH

Copy paste---
%Existing computational work on pattern processing in melodies of Indian art
%music is mostly focused on analysis and modeling of gamakas and characteristic
%phrases of a \gls{raga}. Predominantly these approaches follow a supervised analysis
%framework. In this section we present only a selection of works related to these
%topics. \citeA{Subramanian2012} proposes `longest path' optimization algorithm for phrase-level selection of gamakas and
%two component `stage-dance' representation for modeling gamakas in Carnatic
%music. \citeA{Datta2007} attempt to automatically extract `meend', a class of
%gamakas from audio signals using heuristic rules, music knowledge and curve
%fitting measures. 
