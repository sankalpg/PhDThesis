...
We first review the existing approaches in terms of these three processing units. From~\tabref{tab:pattern_processing_iam} we notice that a majority of these approaches work with a fine grained continuous melody representation. Few studies examine the affect of abstracting the melody representation by using techniques such as SAX and BSS.

, the accuracies are inferior to that using continuous representation. These representations are only evaluated on a particular style of music within a single music tradition (either hindustani or Carnatic), so their scalability to all types of melodic styles within \gls{iam} is questionable. In addition, even with abstracted representation the authors used a dynamic programming based distance measure as there are high degree of timing variations across occurrences of the melodic patterns. For sure, it provides an advantage compared to a continuous representation but still its not as efficient as using a Euclidean distance. Few authors have used an abstracted melodic representation based on the saddle points XXX. They exploit the specific melodic characteristics in Carnatic music. However, such representation is coarse and is mainly used to filter the search space to reduce the computational cost. The final computation is done by a continuous representation. Overall, we see that devising an abstracted melodic representation that can encapsulate time and pitch variations is a challenging task for \gls{iam}. This can mostly be attributed to the complex melodic framework of raga, wherein the melodic transition regions between svaras contain a lot of information about the raga. 

Common evaluation on both hindustani and carnatic, that will make it clear how representation affects the performane in different types of melodic characteristics. No study does that. Even in continuous repreentation there are other aspects to study. Sampling rate no one studies. transposition no one studies!! everone does tonic normalization. Different granularity of ptich resolution is not studied. 

Impication of using a fine grained melody representation.

As mentioned before, an important aspect in pattern detection is melody segmentation. While there are well studied methods for melodic segmentation in symbolic music, such methods for \gls{iam} in audio recordings are almost inexistent. There is a very little work done in terms of melody segmentation for \gls{iam}. As a result of which, approaches for pattern detection in \gls{iam} either use a brute force segmentation strategy or use a distance measure which does not require an explicit melody segmentation (such as Smith Waterman algorithm). There are a few approaches that use specific rhythm and melody landmarks in audio recordings such as Sama locations or \gls{nyas} \gls{svara} onsets to determine the location of the potential pattern candidate. However, these approaches are either too specific to a certain type of melodic pattern or are not scalable to large music collections. For example, Sama location can be an indicator of only Mukhda phrases in an recording and has no musically established relation with the location of characteristic phrases of ragas. The Pa \gls{nyas} segmentation strategy followed in XX can work only with phrases ending in Pa \gls{svara} and for mainly for slow tempo recordings where the concept of \gls{nyas} svar is evident. Moreover, detecting these landmark is not perfect and introduce errors in the system. Note that approaches that address the task of pattern distinction use ground truth annotation for melody segmentation. Their main focus is to study melodic similarity. Overall, we see that there has not been any scalable approach in the literature to melody segmentation in \gls{iam} that can benefit the task of pattern detection and discovery.

Similarity (or dissimilarity) measure is another crucial block in pattern processing tasks. As explained earlier that most of the approaches use a continuous melody representation. In such a scenario the timing and the pitch variations across occurrences of the melodic patterns have to be addressed by the similarity measure. We see that a majority of approaches uses a dynamic programming based similarity measures. This can be attributed to the fact that melodic patterns undergo a large degree of non-linear timing variation in melodies of \gls{iam}. It has been repeatedly shown that such a duynamic programming based distance measure performs better than Euclidean distance XXX. XX uses XXX method for computing similarity as it is suited for a discrete representation of time series. On the other hand XX and XX use XX and XX algorithm, which are more suited for continuous representation of time series. There have been some modifications propsoed in these similarity measures that exploit specific melodic characteristics of this music tradition. XXX proposes a modification to RLCS in which he considers XXX. YYY proposes to use a customized global constrained best suited for each pattern category. However, scalability of these proposals that expoit specific information about a pattern category or of a particular raga is yet to be assessed. Moreover, such methods can not be applied to unseen data, which is often the case in tasks such as pattern discovery. In contrast to the DP based methods there are approaches that consider this task as XXX and utilize HMM to compute melodic similarity. Something more on distance measures...

We now compare existing approaches in terms of other relevant aspects involved in the task. An important consideration in pattern detection for sizable datasets is the computational complexity. It becomes even a bigger concern since the distance measures that performed well  as seen above are mainly based on DP. As seen from~\tabref{tab:pattern_processing_iam} not every approach addresses this concern. One of the ways as proposed by XX is to improve the computational efficiency of the system by using an abstracted low-dimensional discrete melody representation. However, the performance of such a system is inferior to that using a continuous melody representation. In addition, the accuracy of such a system is limited by the performance of melody transcription or any other discretization procedure, which remains a challenging task for melodies in \gls{iam}. Another type of optimization is to perform the task of pattern detection in two stages as proposed by XXX. In the first stage a coarse melody representation can be used to identify the regions in recordings that are more likely to contain the relevant pattern. Using a coarse melody representation can reduce drastically the computational cost. Once the search spaced is pruned, in the second stage, a fine grained continuous melody representation can be used to reliably detect the pattern occurrences. However, the coarse melody representation proposed by XXX is specific to Carnatic music and is not generalizable to Hindustani music. Besides, the approach does not produces a theoretical lower-bound and the pruning is based on an empirically computed threshold. This means that it is not very suitable for unsupervised analyses such as the task of pattern discovery where determining a musically relevant similarity threshold in itself is a challenge. Furthermore, the first stage does not result in a cent percent precision, and therefore, might become a bottleneck for the system. Some other proposals for reducing computational complexity involve top-down melody segmentation by exploiting specific type of melodic and rhythmic landmarks such as sama and nyas onsets. Occurrence of certain types of phrases are marked by these events. However, it makes the system tuned to a particular type of melodic phrases and musical forms within Hindustani music. Scalability of such a system on a diverse dataset becomes limited by these factors.

There are several other shortcomings and venues for improvement in the existing approaches. One of the biggest critic is the evaluation setup. Apart from the evaluation setup being different across studies, even within a study it can be improved a lot. Most of the approaches are evaluated with only a few recordings from a handful of ragas. It might be wrong to believe that few hundreds of annotated patterns represent the real-world diversity present in such a rich improvisatory music tradition. Thus, dataset size and diversity is one of the potential areas of contribution. The evaluation measures used by these approaches in several cases are ad-hoc. For example, XX uses only 5 phrases taken from the starting of the recordings to evaluate the system. Although the other annotated instances if retrieved are regarded as a true hit. Thus, not considering all the annotated pattern instances as a query creates a evaluation bias, which should be addressed by standardizing evaluation setup. Another example is in XXX, the authors measure precision and recall using only top 10 retrieved results. In supervised situations when the number of relevant documents are of the order of XXX, using only top 10 results for computing evaluation metric biases the results. Thus, there is a need to promote well established evaluation metric often used in information retrieval task to make the results more comparable. 

Based on our discussion above we see that there are a variety of approaches proposed for melodic pattern processing in \gls{iam}. However, a direction comparison across these approaches cannot be made, since these approaches are evaluated using different datasets and different evaluation measures. The approaches that perform pattern distinction encounter far less number of noisy candidates in the search space as compared to the approaches that perform pattern detection. Thus, due to the varied complexity of the task, the results reported in these studies cannot be directly compared. A systematic improvement in melodic pattern processing task calls for an comprehensive comparative evaluation of these approaches to understand their limitation and strengths for different type of music content. Another observation is that there is only one method that addresses pattern discovery. That too work with short duration first line of the compositions specifically recorded for the study. Scalability of such an approach on hundreds of hours of data is questionable. Understandably pattern discovery a complex task compared to pattern detection. Nonetheless there should be more efforts put on that specially because dataset size for supervised approaches tends to be not representative of the real-world collection. Also, the systems might be tuned to a particular type of melodic patterns or biases of the musicians. 

Pattern characterization is not seen nicely?? No evaluation of sampling rate? octave normalization?? discovery method does not address scalability issues and redundancy filtering at all...the motif is identified as the most repeating pattern, however the material used was synthetic, in real world discovery results in a lot of rendant patterns where the most often occuring might not tbe the characteristic. THus we need approaches to characterize the discovered melodic patterns. We need to exploit culture specific traits to improve similarity and not pattern specific.
