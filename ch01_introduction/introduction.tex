...
\Gls{it} is constantly shaping the world that we live in. The advancements in \gls{it} has changed the patterns of human behavior and influenced the way we connect with our environment. The ease with which digital content is generated and shared, further propels the growth of these technologies. Music being a universal socio-cultural phenomena has been deeply influenced by these advancements. The way music is created, stored, disseminated, listened, and even learnt has undergone severe changes. A massive amount of audio music content is now available on demand. In order to facilitate interaction with such large volumes of digital music content It becomes necessary to develop computational techniques that can automatically describe and deal with this digital music content in a meaningful way. 

Melody is a fundamental music dimension, along with harmony and rhythm, and thus, an indespensible component in the description of music. As put by XXX

YYYY. Melody is organized in heirratchies .



2) Why melody is important and complex and useful in music description
2.5) Hierarchical organization of melody, at diff levels
3) How melody is typically described in terms of what elements at what levels
4) Done for western music, On audio its hard to get representation. Indian art music. easy melody and complex concepts, awesome use case



5) Importance of patterns, 
6) Interesting concept of raga




%Applications
1) Music organization, structuring, recommendation
2) Music search, indexing, raga based retrieval
3) Pedagogy, Understanding how experts have taken the approach of raga, how a raga evolve through a piece, in theory there are no explicit rules, students learn through years of practise, musician's can't bervalise
4) Enhanced listening experience. 
5) Film music influence, big market, millions of listeners, millions of leaners/
...
\subsection{Computational Melodic Description Indian Art Music Melodies}
...
\COMMENT{patterns can be used for music analysis, characterizing ragas compositions and artists. They can be used to study influences of artists, gharanas, since its a oral tradution its an interesting topic. They can be used to study evolution of music and how the usage of patterns has been changing. They can be used to perform intra piece analysis, how a piece evolves, different improvisatory aspects within a piece. Since artist puts in his mind, his characterization. Then it can be used to build other computation tasks, raga id, discovery of music, recommendation etc. Then it can be used to build applications music pedagogy etc.}
...
\TODO{here quote the raga is between scale and melodies etc. THus a unique kind of an opportutnity to study both the aspects. Studying raga recognition we get to learn about them. THus we do that..Why raga, motivation, and then its advantages, computatinally and musicologically. .FUll music is based around raga, its the central most important concept, its the most rich annotation one  can produce. So we go for it, its the best description fo the piece of music.}
...
%These challenges help expose the shortcomings of the current \gls{mir} systems. Devising novel approaches that overcome these challenges will not only result in making \gls{mir} more accessible to \gls{iam}, but it will also push the boundaries of \gls{mir} technologies. 

%Such computational approaches can enable \gls{raga}-based music retrieval from large audio collections, semantically meaningful music discovery, musically informed navigation and several applications around music pedagogy. 
%
%In the next section we provide a detailed overview of the challenges and opportunities offered by \gls{iam} in computational analysis of its melodies, which further adds to our motivation behind selecting \gls{iam} for this study.
