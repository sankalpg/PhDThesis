...
\begin{figure}	[h]
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=\figSizeEighty]{ch07_ragaRecognition/figures/Accuracy_Vs_Size_n_raagas_cmd.pdf}
		\caption{For subsets of \acrshort{rrds_cmd_big} dataset}
		\label{fig:performance_vs_n_raagas_cmd}
	\end{subfigure}
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=\figSizeEighty]{ch07_ragaRecognition/figures/Accuracy_Vs_Size_n_raagas_hmd.pdf}
...
In~\figref{fig:performance_vs_n_raagas} we show a boxplot of the \gls{raga} recognition accuracies obtained for different subsets of both the datasets over multiple iterations of the experiment. A general observation is that the accuracy decreases as the number of \glspl{raga} in the dataset increase, which is explicable as the task becomes more challenging. Notably, the decrease is more significant in the case of data subsets comprising Carnatic music~(\figref{fig:performance_vs_n_raagas}\,(a)) as compared to the Hindustani music. This indicates that the task of \gls{raga} recognition 

In FIG XXX we show the accuracies for subsets of XXX. We notice that the accuray varies quite significantly and the difference between the minimum accuracy and the maximum accuracy for a particular number of ragas is around 10\%. That means that some subset of the XXX with the same number of ragas can result into 10\% more accuracy compared to another subset with the same number of ragas. This indicates that the performance isquite sensitive across different subsets. Over the number of ragas, we see that accuracy drops as the number of ragas increase. 
