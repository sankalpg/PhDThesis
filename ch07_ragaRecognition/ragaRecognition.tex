...
\tablebot
\end{tabular}	
	\caption[Accuracy of \acrshort{ragarecVSM}, \acrshort{sotaChordia} and \acrshort{sotaKoduri} on \acrshort{rrds_cmd_big}]{Accuracy (in percentage) of \gls{raga} recognition on \acrshort{rrds_cmd_big} dataset by \acrshort{ragarecVSM} and other methods methods using different features and classifiers. Bold text signifies the best accuracy by a method among all its variants.\TODO{I am not able to reproduce Gopala's results, even after getting the original set of features from him. Waiting for his response.}} 
...
\begin{table}[t] 
	...
	\caption[\Gls{raga} recognition accuracy of \acrshort{tdms}-based method variants
	]{Accuracy (\%) of the three proposed variants, \acrshort{ragarecTDMS_F}, \acrshort{ragarecTDMS_KL} and $\mathcal{M}_{\mathrm{BC}}$, and two other methods \acrshort{sotaChordia} and \acrshort{ragarecVSM}. The random baseline ($\mathfrak{B}_\mathrm{r}$) for this task is 3.3\% for \acrshort{rrds_hmd_big} and 2.5\% for \acrshort{rrds_cmd_big}.}
...
\begin{figure}[h]
	...
	\end{center}
	\caption[Accuracy of \acrshort{ragarecTDMS_KL} as a function of different parameter values]{Accuracy of \acrshort{ragarecTDMS_KL} as a function of parameter values. Other methods \acrshort{sotaChordia} and \acrshort{ragarecVSM}, and random baseline $\mathfrak{B}_\mathrm{r}$ are also reported for comparison.\TODO{Match $\mathfrak{B}$, wasn't happening in matplotlib}} 
