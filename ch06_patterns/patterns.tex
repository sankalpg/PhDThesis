...
\COMMENT{if time permits you can write a equation for each of them.}
...
\tablebot		
	\end{tabular}
	\caption{\acrshort{map} score and the details of parameter settings for the three best performing variants for the \acrshort{msds_iitm_cmd} and the \acrshort{msds_iitb_hmd}. Srate: sampling rate of the melody representation, Norm: normalization technique, TScale: uniform time-scaling and  Dist: distance measure.}
...
The \gls{map} scores and the details of parameter settings for the \acrshort{msds_iitb_hmd} dataset are shown in~\tabref{tab:melodic_similarity_results} (bottom half). Compared to the \acrshort{msds_iitm_cmd}, the best \gls{map} score for the \acrshort{msds_iitb_hmd} is higher (0.55). Amongst the top ranked variants there is no consensus on the sampling rate of the melody representation. All the top ranked variants have the same parameter values except the sampling rate. This suggests that the sampling rates considered in this study have no significant effect on the melodic similarity for the \acrshort{msds_iitb_hmd}. This can be attributed to the fact that the recordings in the \acrshort{msds_iitb_hmd} are slow-medium tempo music pieces that do not have fast oscillatory melodic movements, as was the case with the \acrshort{msds_iitm_cmd}.  Furthermore, for the \acrshort{msds_iitb_hmd}, we observe that the variants using $\mNorm_{\mathrm{tonic}}$, $\mNorm_{\mathrm{tonicQ12}}$ or $\mNorm_{\mathrm{tonicQ24}}$ perform better than the ones using $\mNorm_{\mathrm{mean}}$, which is in contrast to the observation for the \acrshort{msds_iitm_cmd}. This is primarily because in Carnatic music in our datasets there are many cases where a pattern recurs in a different octave within the same recording, whereas, in Hindustani music, such cases are rare. In general, we see that the \gls{dtw}-based distance performs better than the euclidean distance, and the \gls{dtw} variant without a global constraint ($\distPatt_{\mathrm{DTW\_L1\_G90}}$ or $\distPatt_{\mathrm{DTW\_L0\_G90}}$) is preferred. This implies that the repeated instances of melodic patterns in IAM (specifically in Hindustani music) have large non linear timing variations.
...
\TODO{solic motivation for discovery, read papers man, there are nice thigns written about this}
...
\TODO{write a better sota after reading more!. Reseachers have repeatedly said that DTW is infeasible, that could be a reason to try it our and exploit the advancements in time series analysis}
\TODO{Pattern extraction for melodies in western music when there is so much theory about segmentation is tough, imagine for indian music where there is no computatinoal ground for such steps. It becomes really complex. Emilia and Klapuri (JNMR) rejected out the possibility of brute force approach. However advancements in speeding up distance measures in other areas such as time series can be used for brute force approach. Its true that it invoves compromises in terms of pattern length but thats also is in the process of getting solved. Besides even for reasonably big dataset if we have patterns they can be used to stufy and gain insights into segmentation. THis way it can helop in developing segmentation approaches and bootstrap the task of pattern extraction. This is a nice perspective and justification of our approach}
...
\TODO{A vast majority of patterns and repetision in music might not be musically or perceptually significant, and thus we need ways to identify them..big challenge is how to detect significant ones. an that is what this chapter is about!!! enjoy :) One can't model them explicitly specially in discovery cases. There might be a wide range of significant patterns which do not exhibit explit behavoir ? indicating them being significant}
...
\TODO{There are quite many examples you can get for characterizaiton and ranking of motifs, look at the literature in symbolic domain, Conklin and Collins might have done soem work. Klapuri and Emila have mentioned two ways of going about compting similarity. Encoding musical info in the represenation and then using generic, or using music specific distance measures. So since in our case we want scalable solution for which we have to take a generic dist measure, we encode the musical info in the representation.}
